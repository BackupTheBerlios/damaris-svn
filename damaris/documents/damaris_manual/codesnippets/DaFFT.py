...
class FFT:
	...
	def write_n(self, afile):
		...
		for i in range(self.data_points):
			filename.write('%e\t%e\t%e\n'%(self.the_result.x[i], 
											self.the_result.y[0][i], 
											self.the_result.y[1][i]))
		filename.close()	
		return self
	...
	def base_corr(self, cutoff=0.3, show=0):
		...
		subtracted.
		If show=1 the result is return and not the instance. 
		This allows to plot the baseline corrected signal
		Example:
		...
		for i in range(2):
			self.the_result.y[i] = (self.the_result.y[i] - 
								self.the_result.y[i][:-last_points].mean())
		if show == 1 :
