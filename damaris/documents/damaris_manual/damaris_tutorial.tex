...
Sometimes the detected signals intensity is low compared to the noise, and can therfore not be seen. A measure for the quality of the recorded signal is the signal-to-noise ratio (SNR) which is defined as the ratio of signal versus the standard deviation of the noise, i.e. noise strength. 
...
Artefacts due to imbalance in the receiver, inhomogeneity in the RF field and unwanted signals, for example primary echos, can be suppressed or cancelled out by cycling the phases \citep{Berger:2004vn} of the receiver and the RF pulses and then accumulating the signal.
...
To store the result for further data analysis one has several built-in possibilities in \textsf{DAMARIS}. One can store the results as \textsf{ASCII} file or as \textsf{HDF5}\footnote{http://hdf.ncsa.uiuc.edu/HDF5} file. Writing  an \textsf{ASCII} file is usable for smaller data-sets and a small number of experiments. 
...
The speed-up in post-processing and analysis is already 24 fold in small data-sets with 32768 points per channel and 25 files on a Athlon XP 2600+ CPU with 512 MB Ram running Debian Sarge Linux. Thus writing to \textsf{HDF5} with compression enabled is highly recommended.
...
A very convenient and easier method to save data in HDF file is the MeasurementResult class.
\begin{lstlisting}
import MeasurementResult
...
def fid():
	# create a measurement object
	measurement=MeasurementResult.MeasurementResult("Magnetization(tau)")
	for timesignal in results:
		...
		...
\end{lstlisting}
Eessentially, a dictionary ``measurement'' is created with \emph{tau} as the key. For each new key the current magnetization is added. This dictionary is then added to the data pool. The advantage over Listing \ref{advancedhdf} is that the magnetization vs. $\tau$ curve can be directly displayed in order to gain an overview of the running experiment, because the ``measurement'' dictionary is added to the data pool. In this example, the value also stores statistics from the accumulation. The data dictionary is stored automatically as described before, including the result and experiment scripts, information about the used back end and a timeline of the experiment.

\section{Fourier Transform Module}
A module was written for an easy calculation of spectra from time domain data. The Fourier transformation module ``DaFFT'' is based on the \textbf{fft} module of \textbf{numarray}, an array module for Python. There are no interactive capabilities such as phase correction incorporated yet. After creation of the FFT object, which contains a copy of the timesignal, several methods can be applied to the object (see Listing \ref{fftex}). The order in which these methods are applied \emph{does} matter. Here are the available methods and a short explanation of their effects is shown (values in italic are default values).

\begin{description}

\item[base\_corr(cutoff=\textit{0.3}, show=\textit{0})]
This method should be applied first as it corrects the baseline of the signal, taking the last \textbf{cutoff} part of the signal for determining the correction. If \textbf{show} evaluates to \textit{True} the result is returned, otherwise the object itself for further modification. This is practical for determining the effects of any function to the timesignal immediately.

\item[abs\_fft(points=\textit{None}, zoom=\textit{None})]
...
\item[fft(points=\textit{None}, zoom=\textit{None}]
This method returns the real and the imaginary part of an FFT of the timesignal. Zero-filling of the timesignal can be done via the \textbf{points} keyword. If \textbf{points} is bigger than the number of data points, the rest will be filled with zeros (zero-filling/zero-padding).
\end{description}
%
%\begin{itemize}
...
\begin{description}
\item[exp\_window(LB=\textit{10}, show=\textit{0})]
...
\item[gauss\_window(LB=\textit{10}, show=\textit{0})]
This sets a gaussian window over the data:
\begin{equation*}
f(x)=exp(-(tp \cdot LB)^2)
\end{equation*}

\item[dexp\_window(LB=\textit{-10}, gaussian\_multiplicator=\textit{0.3}, show=\textit{0})]
...
\begin{equation*}
f(x)=exp(-tp\cdot LB - gaussian\_multiplicator\cdot  tp^2)
\end{equation*}

\item[traf\_window(LB=\textit{10}, show=\textit{0})]
...
\begin{itemize}
\item hanning\_window(show=\textit{0})
\item hamming\_window(show=\textit{0})
\item blackman\_window(show=\textit{0})
\item bartlett\_window(show=\textit{0})
\item kaiser\_window(beta=4, show=0, use\_scipy=None)
\end{itemize}

To use this methods one has to create a DaFFT object and apply consecutively applicable methods:
\begin{lstlisting}[label=fftex]
import DaFFT
fftdata = DaFFT.FFT(timesignal)
data["FFT"]=fftdata.base_corr(0.4).exp_window(line_broadening=100).abs_fft(zoom=(800,1000))
\end{lstlisting}
On the second line of this example, the fftdata object is created where the methods will be applied to.
The third line  applies baseline correction using the last 40\% of the timesignal for determination of the signal. Then, an exponential window is applied. From this timesignal an absolute FFT is calculated, returning a view to the data at \unit[800]{Hz} with \unit[1000]{Hz} width.
...
\begin{lstlisting}
def experiment():
	for tp in Experiment.lin_range(start=5e-7, stop=10e-6, step=5e-7):
...
\begin{lstlisting}
def experiment():
	for t in Experiment.log_range(start=5e-3, stop=10, stepno=10):
...
\begin{lstlisting}
def experiment():
	myrange=Experiment.log_range(start=5e-3, stop=10, stepno=10)
	for tp in Experiment.staggered_range(myrange, size=2):
		yield inversion_recovery(tau=t)
\end{lstlisting}
...
\begin{lstlisting}
def experiment():
	mylinrange=Experiment.lin_range(start=1e-3, stop=4.9e-3, step=1e-4)
	mylogrange=Experiment.log_range(start=5e-3, stop=10, stepno=10)
	for tp in Experiment.combine_ranges(mylinrange, mylogrange):
		yield inversion_recovery(tau=t)
\end{lstlisting}
...
Following is a list of available commands in a result script. These are methods to be applied to a ADC\_Result, Accumulation or DaFFT  object. Accumulation and DaFFT objects need to be created before any methods can be applied.
\newline
\emph{\textbf{Example}}:
\begin{lstlisting}
import Accumulation
akku = Accumulation.Accumulation(error=True)
akku += timesignal
\end{lstlisting}
...
\item[write\_as\_csv(destination=\textit{sys.stdout})]
...
timesignal.write_as_csv(destination=f)
f.close()
\end{lstlisting}
